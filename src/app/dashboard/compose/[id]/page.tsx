...
import { Upload, X, Calendar, Send, FileText, Trash2, Image as ImageIcon } from "lucide-react";
...
export default function EditPostPage() {
  ...
  useEffect(() => {
    async function loadPost() {
      ...
      setUploadedFiles(
        (data.media_urls as string[]).map((url, i) => ({
          id: `existing-${i}`,
  ...
  async function handleUpdate(status: "draft" | "scheduled" | "published") {
    if (!content.trim()) {
    ...
    if (status === "scheduled" && !scheduledAt) {
      toast({ title: "Schedule time required", variant: "destructive" });
      return;
    }
    ...
    <div className="p-8 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Update your scheduled content</p>
        </div>
        <Button
          variant="destructive"
          size="sm"
              ...
              <div>
                ...
                >
                  ...
                  <p className="text-sm text-gray-500">
                    {uploading ? "Uploading…" : "Drop images/videos here, or click to add more"}
                  </p>
          ...
          <Card>
            <CardContent className="pt-6 space-y-3">
              <Button
                ...
                onClick={() => handleUpdate(scheduledAt ? "scheduled" : "published")}
              >
                <Send className="h-4 w-4" />
                {scheduledAt ? "Update Schedule" : "Update & Publish"}
              </Button>
              <Button
                variant="outline"
                className="w-full"
